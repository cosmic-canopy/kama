...
# ...and the other half: the EXACT lines, which is the only way to catch a wrong line that happens to be
# inside the file. Measured on this corpus by rendering every diagnostic one line off: the range check
# above catches 77 of 557 fixtures, so 86% of an off-by-one walks straight through it.
#
...
# and satisfy yourself each moved line is now MORE right, then commit it as part of that change.
diag_lines() {   # diag_lines <errfile> <fixture.kama>...  -> "a.kama:3 a.kama:7"
    local err="$1" s sb; shift
    for s in "$@"; do
        sb="${s##*/}"
        LC_ALL=C awk -v sb="$sb" '
            { i = index($0, sb ":"); if (i == 0) next
              rest = substr($0, i + length(sb) + 1)
              if (rest !~ /^[0-9]+:/) next
              seen[sb ":" (rest + 0)] = 1 }
            END { for (k in seen) print k }' "$err"
    done | sort -u | tr '\n' ' ' | sed 's/ *$//'
}
...
xfail_one() {
    ...
    # shellcheck disable=SC2086
    got=$(diag_lines "$err" $fsrcs); [ -n "$got" ] || got="-"
    if [ "${KAMA_UPDATE_DIAG_LINES:-0}" = 1 ]; then
        printf '%s\t%s\n' "$name" "$got" >"$TMP/dl_$name"
    else
        want=$(LC_ALL=C awk -F'\t' -v n="$name" '$1 == n { print $2; found=1 } END { if (!found) print "\001" }' \
        ...
        if [ "$want" != "$got" ]; then
            { echo "FAIL xfail/$name (a diagnostic MOVED)"
              echo "  recorded: $want"
              echo "  now:      $got"
              echo "  If the fixture was edited, regenerate. If it was not, a diagnostic changed where it"
              echo "  points — read the new line before accepting it. KAMA_UPDATE_DIAG_LINES=1 ./dev test"; } >"$out"
            echo FAIL >"$res"; return
        fi
    fi
...
phase_end
# Assemble the golden record from the per-fixture pieces the workers wrote. Sorted, so the file is stable
...
#
# Why it exists: `.msg` asserts a MESSAGE, and nothing asserted a POSITION, so every fixture in this
# directory would have passed with every line wrong. The range check beside it catches only a line outside
# the file — measured at 77 of 557 fixtures for an off-by-one. This catches the other 86%.
#
# ⚠️ REGENERATING IS NOT A FIX. A row moves for two reasons: the fixture was edited, or a diagnostic
