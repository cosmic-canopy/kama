...
| **Reflection / metadata** | ✅ shipped as opt-in **serialization** — `@generate(Serialize, Deserialize)` on a type/enum drives the compiler's `ClassInfo` (field name/type/order + sum-type variants) into synthesized `serialize`/`deserialize`; one uniform contract (intrinsics + collections + user types + enums) + a `std::serialization::json` backend (`encode`/`decode`). Remaining: object-graph (pointer-as-id) mode; more back ends (YAML/binary/XML). See [SPEC.md](SPEC.md) "Serialization". | Auto-serialization, editor property panels, ECS introspection. | ✅ (graph + back ends follow) |
