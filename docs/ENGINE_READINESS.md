...
| **comptime / const-eval** | 🟡 partial — `const` values, **const generics** (`InlineArray<T,N>`, integer type params), **`sizeof(T)`/`alignof(T)`**, **const-param arithmetic (6b-1)**, and **named `comptime` constants at local/module/type scope (6b-2, `comptime T NAME` / `Type::NAME`)** are shipped — they drive const-generic sizes and bake `static const` data; only compile-time *function* evaluation (`comptime fn` — the lookup-table generator) is left (its own campaign, ROADMAP §5). | Lookup tables, shader/permutation specialization, asserts. | M |
