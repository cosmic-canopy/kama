...
- Generic **collections** (`FixedArray<T>`/`DynamicArray<T>`/`string`, bounds-checked, monomorphized) and **user
  ...
  self-type, turbofish `f::<T>()`) — plus **comptime parameters** and the **`InlineArray<T>#(N)` safe fixed array**
...
*substrate* exists and only the higher-level scheduler *library* remains (ROADMAP_DETAIL §6). (Slices/spans
...
| SIMD / no-heap | **auto-vectorization, on every target** — `std::math`'s types carry a SIMD-ready contiguous layout and `--release` inlines the ops so the C backend packs them (measured: `fadd.4s` loops, and `ld4`-based AoS→SoA for `dot`/`Mat4.transform` on native; `f32x4`/`v128` on wasm, which passes `-msimd128`). Both halves are guarded by `tools/check-simd-native.sh` / `check-simd-wasm.sh`, which read real machine code. There is **no** `std::simd` module and no explicit vector type or intrinsic, so a **shuffle or a lane mask has no spelling** — see [SPEC.md](SPEC.md) *Math*. `@noheap` / `--no-heap` for frame allocators and audio callbacks; **`@align(N)`/`@packed`** state a struct's layout for a vertex buffer or an std140 block |
...
|---|---|---|---|
| **FFI depth**: `extern` structs, opaque handles, function-pointer params, pass structs by value/ptr to C, map/include C headers | ✅ — `extern` structs, `extern "<header>"` includes, opaque `UnsafePtr<T>`, `addr(of:)` out-params, and C-callback fn-pointers all shipped; residual is richer struct-by-value ergonomics | WebGPU, SDL/GLFW, platform, audio are **C APIs**. The keystone — landed. | done |
...
| **Math layer**: vector/matrix/quaternion types + SIMD | ✅ **DONE** — **`std::math` shipped**: concrete float32 `Vec2/3/4`, `Mat2/3/4`, `Quat` + scalar helpers (`import { std::math::Vec3, … };`). Column-major, WebGPU 0..1-depth `perspective`/`ortho`/`lookAt`, Mat4 `inverse`, full `Quat` (Hamilton/`rotate`/slerp/`toMat4`), method chaining. Native + ASan + wasm, exact-value fixtures. **SIMD — native ✅, wasm ✅**: the SIMD-ready layout auto-vectorizes at `-O3` and `--release` unity-TU builds inline the ops, so hot math runs **at C parity** — measured on aarch64 (NEON) and on the x86-64 SSE2 baseline, and clang goes further than elementwise packing, de-interleaving AoS to SoA for `dot` and `Mat4.transform`. **wasm** gets it too: the driver passes `-msimd128` on every wasm build, so a `.wasm` carries `f32x4`/`v128` (it carried **zero** until 2026-08-30 — the flag had never been passed, and nothing measured it). Both tiers are now guarded by `tools/check-simd-native.sh` / `check-simd-wasm.sh`. The **shuffle** and the **lane mask as a value** — the two operations auto-vectorization cannot express — now have one: the explicit `Simd<T> comptime(int32 N)` type, with `Mask<T>#(N)`, integer lanes and a derived `SIMD128` flag ([SPEC.md](SPEC.md) *Explicit SIMD*), guarded by `tools/check-simd-type.sh`. | Transforms, physics, culling, shading — the numeric core. | done |
| **InlineArray-size value arrays** (`float[4]`, matrix storage) | ✅ — **`InlineArray<T>#(N)`** shipped: a bounds-checked value array (`struct{T v[N];}`, monomorphized per (T,N), value-copy, no pointer decay), with array literals `[a,b,c]`/`[v;N]`, `.length()`, `foreach`, nested `InlineArray<InlineArray<T>#(M)>#(N)`, and place-indexing. `Mat4 = InlineArray<float32>#(16)` or `InlineArray<Vec4>#(4)`. Const OOB is a compile error; dynamic OOB traps. | Compact stack numeric storage, SIMD lanes, ergonomic matrices. | done |
...
| **File / network I/O** (files, sockets) | ✅ — **`std::fs`/`std::io`/`std::net` shipped** (RAII `File`, `readFile`/`writeFile`/`stat`/`readDir`; blocking TCP `TcpListener`/`TcpStream`; `Result<…,IoError>`), POSIX + Windows, over the bundled `kama_os.h` FFI boundary. `examples/httpd/` is a real static-file server on it. Follow-ups (UDP/DNS, buffered readers) tracked in ROADMAP_DETAIL §1. | Asset/scene loading, config, tooling, networking. | done |
| **String formatting / interpolation** (`"${x}"`, number→string, logging) | ✅ — **shipped**: the `Formattable` contract + `Formatter` sink (prelude), plus `${expr}` interpolation (identifier + `.field`/`[index]` holes) lowering to a compile-time, statically-checked `Formatter` build — one buffer, no O(n²) concat. On top of the already-rich `string` (ops, `split`/`trim`/…, UTF-8 + `.chars()`). Follow-ups (additive): format specifiers `${x:.2f}`, a `@generate` debug derive, tagged strings (`sql"…"`/`html"…"`) — see [SPEC.md](SPEC.md) "Formatting & string interpolation". | Logging, text assets, tooling. | done (specifiers/tags follow) |
| **comptime / const-eval** | 🟢 shipped — `const` values, **comptime parameters** (`InlineArray<T>#(N)`, integer type params), **`sizeof(T)`/`alignof(T)`**, **comptime-param arithmetic (6b-1)**, **named `comptime` constants at local/module/type scope (6b-2)**, and **compile-time *functions* `comptime fn` (6b-3)** all ship. A `comptime fn` RUNS at build time and bakes its scalar/table result into a `static const` — a lookup table / permutation constant computed once by the compiler, zero runtime cost. Bounded + pure; comptime-only; both top-level and type-associated (`Type::name()`, member-visibility-controlled). Flagship `comptime_fn_crc` (baked CRC-8 LUT). | Lookup tables, shader/permutation specialization, asserts. | M |
| **Reflection / metadata** | ✅ opt-in **serialization shipped** — `@generate(Serializable, Deserializable)` on a type/enum drives the compiler's `ClassInfo` (field name/type/order + variants). User surface = contracts + attributes (+ hand-written override); the field walk **and** the object-graph rebuild are a **compiler intrinsic** (lowering to C), with swappable library wire backends (`std::serialization::text::json` now). Two modes gated on `reachesPointer`: pointer-free ⇒ by-value (`deserializeJsonBuffer::<T> -> T`); reaches a pointer ⇒ heap graph (`deserializeJsonBuffer::<Shared<T>>`), incl. polymorphic `Shared/Weak/Owned<Contract>` + a `DeError` set. Remaining is additive library work (more wire back ends) — see [ROADMAP_DETAIL.md](ROADMAP_DETAIL.md) §4 / [SPEC.md](SPEC.md) "Serialization". | Auto-serialization, editor property panels, ECS introspection. | done (json; more back ends follow) |
...
through `Optional` doesn't monomorphize yet, ROADMAP_DETAIL §5).

## The actual goal

| Goal | Status | Notes |
|---|---|---|
| **kama-level WebGPU bindings** → first triangle → (engine spine) | 🟡 — **triangle shipped, browser/WASM *and* native desktop** (`8db3408`). [`examples/webgpu/`](../examples/webgpu/) is a **pure-Kama** WebGPU triangle (instance→adapter→device→queue→surface→render pass→draw), no C glue: opaque handles are `UnsafePtr`, C descriptors are `type extern value` (header owns layout), the async adapter/device handshake rides `fnptr` callbacks + `userdata`, the frame loop is `std::app`. The `std::gpu` platform seam ([`lib/std/gpu/kama_gpu.h`](../lib/std/gpu/kama_gpu.h)) abstracts surface/present/pump so one source builds web (canvas) and native (GLFW + Metal/X11/Wayland/HWND); `examples/webgpu/build-native.sh`. **kama-scoped remainder:** a thin safe `std::gpu` binding wrapper (handles→RAII, as `std::net` wraps sockets) plus a typed acquire result — **no longer "optional polish"** (the safe `std::gpu` wrapper row): the first external project's frame loop leaked 15.6 GB and took a machine down twice because a NULL texture carried an untyped `status` int nobody read. ⚠️ It was the *typed result* that would have caught that one, not RAII; RAII answers the five hand-written per-frame `wgpu*Release` calls in the example. Still not a 1.0 blocker. **The engine *spine* (buffers/bindings/pipelines/renderer beyond a triangle) is the engine *product* built on kama, not part of kama — see [ROADMAP_DETAIL.md](ROADMAP_DETAIL.md) §8; out of scope for the language.** | seam shipped; safe wrapper ROWED, not polish |
