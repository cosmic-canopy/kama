...
| **comptime / const-eval** | 🟡 partial — `const` values, **const generics** (`InlineArray<T,N>`, integer type params), and **`sizeof(T)`/`alignof(T)`** (monomorphizing compile-time builtins, both shipped) are done; general compile-time *evaluation* (arithmetic on const params, lookup-table generation) is not. | Lookup tables, shader/permutation specialization, asserts. | M |
