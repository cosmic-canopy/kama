...
| SIMD / no-heap | **auto-vectorization, on native** — `std::math`'s types carry a SIMD-ready contiguous layout and `--release` inlines the ops so the C backend packs them (measured: `fadd.4s` loops, and `ld4`-based AoS→SoA for `dot`/`Mat4.transform`). ⚠️ **Not on wasm** — the release build passes no `-msimd128`, so a `.wasm` today holds **zero** v128 instructions; [design/simd.md](design/simd.md) is the fix. There is **no** `std::simd` module and no explicit vector type or intrinsic, so a **shuffle or a lane mask has no spelling** — see [SPEC.md](SPEC.md) *Math*. `@noheap` / `--no-heap` for frame allocators and audio callbacks; **`@align(N)`/`@packed`** state a struct's layout for a vertex buffer or an std140 block |
...
| **Math layer**: vector/matrix/quaternion types + SIMD | ✅ **DONE** — **`std::math` shipped**: concrete float32 `Vec2/3/4`, `Mat2/3/4`, `Quat` + scalar helpers (`import { std::math::Vec3, … };`). Column-major, WebGPU 0..1-depth `perspective`/`ortho`/`lookAt`, Mat4 `inverse`, full `Quat` (Hamilton/`rotate`/slerp/`toMat4`), method chaining. Native + ASan + wasm, exact-value fixtures. **SIMD — native ✅, wasm ❌**: on native the SIMD-ready layout auto-vectorizes at `-O3` and `--release` unity-TU builds inline the ops, so hot math runs **at C parity** — measured on aarch64 (NEON) and on the x86-64 SSE2 baseline, and clang goes further than elementwise packing, de-interleaving AoS to SoA for `dot` and `Mat4.transform`. On **wasm** the release build passes no `-msimd128` and emits **no** vector instructions at all. What still has no spelling anywhere is a **shuffle** or a **lane mask as a value** — [design/simd.md](design/simd.md). | Transforms, physics, culling, shading — the numeric core. | done |
