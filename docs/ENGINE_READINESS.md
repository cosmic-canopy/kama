...
| **Allocator control**: arenas / pools / frame & stack allocators | 🟡 partial — **M10a shipped**: containers take a copyable-value-handle allocator param `A: Allocator = GlobalAllocator` (à la C++ `pmr` / Zig `std.mem.Allocator`); `DynamicArray`/`Map`/`Set` route every buffer through it via a **direct monomorphized call**, and a caller-owned **`Arena`** + **`BumpAllocator`** give O(1) bump-reset per frame (`DynamicArray::withAllocator(allocator: arena.handle())`). M10b threads the rest of the direct-`malloc` containers. **The gap that keeps this partial:** heap-*boxed* objects (`new T` / `Owned`, hence `SortedMap` B-tree nodes) still draw from the global heap — the frame-arena story is **whole only after allocator-aware `new` / `Owned<T,A>`** (the tracked M11 milestone), so a system can arena-back *all* its per-frame allocations, boxed objects included, and bulk-reset. Fallible `allocate -> Optional<Ptr>` rides the same seam with the embedded no-heap target. See ROADMAP §5.3. | Deterministic per-frame perf, zero mid-frame `malloc`, bump-reset allocators. | M (M11 for `new`/`Owned`) |
