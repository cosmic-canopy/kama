...
6. §2b the Windows seam stops allocating per path (KR-58) — still open, and now measurable rather than
   asserted: `--no-heap` derives `kama__wpath`'s allocation from the header instead of taking a mark's word.
