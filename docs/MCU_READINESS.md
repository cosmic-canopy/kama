...
| **Compile-time evaluation (const-eval)** | 🟡 partial — `const` values + const generics + `sizeof`/`alignof` exist; **arithmetic on const params now folds (step 6b-1)** — `InlineArray<T, (N+1)>`, `2*N` sizes resolve at instantiation (fixture `const_generic_arith`); named-const refs in const-init (6b-2) and compile-time function calls / generated lookup tables (its own campaign, on ROADMAP) do not yet | Baud-rate divisors, gamma/trig tables, permutation constants baked at build time. | **M** |
