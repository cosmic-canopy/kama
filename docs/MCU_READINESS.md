...
| **Compile-time evaluation (const-eval)** | 🟡 partial — `const` values + const generics + `sizeof`/`alignof` exist; **arithmetic on const params folds (6b-1)** and **named `comptime` constants at local/module/type scope (6b-2) ship** — `comptime int32 CAP = 64;` / `Type::SIZE` drive const-generic sizes & static tables, baked to literals with no init-order hazard (fixtures `comptime_module`/`comptime_type`/`comptime_local`); only compile-time *function* evaluation / generated lookup tables (`comptime fn`, its own campaign on ROADMAP §5) is left | Baud-rate divisors, gamma/trig tables, permutation constants baked at build time. | **M** |
