...
| **Soft-float mode / fixed-point** | 🟡 partial — **NEITHER is a language gap** (scheduled next, [ROADMAP.md](ROADMAP.md) §1 near-term order #1). *Fixed-point* = a pure **library** `type value` (e.g. `std::num` `Q16_16`) over the shipped operator overloading — no compiler work. *Soft-float* = a **toolchain flag**, not kama: the emitted C's `float` ops become libgcc/compiler-rt libcalls automatically when the cross-compiler targets a no-FPU core (`-mfloat-abi=soft`/`-mcpu=cortex-m0plus`); make it turnkey with a no-FPU board preset in `mcu/build.sh` (QEMU `microbit` = Cortex-M0). | Cortex-M0/AVR have no FPU. | **S (lib) + toolchain preset** |
