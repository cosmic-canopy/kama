...
over `i` are ordinary, because the counter dies with its loop.
A *parameter* sharing a field's name — the `this.x = x` constructor idiom — is
allowed; a static method has no `this`, so a local there can never shadow a field, and neither exemption is
narrowed by the rule above.
...
a layout both sides agree on, and the marker on the type says **who owns that layout** — exactly as `extern fn`
and `expose fn` say who owns a body:

- **`type extern value`** — a C header owns it, and kama binds it (checked field by field, above). It crosses
  either kind of function: an `extern fn`'s prototype is the header's own, and a host calling an `expose fn`
  includes the same header.
- **`type expose value`** — kama owns it, and the generated host header publishes it (*Exposing to a host*). It
  crosses only an `expose fn`, where kama owns the function too. On an `extern fn` it is refused: that <!-- xfail: expose_value_on_extern_fn -->
  prototype comes from a C header that declares its own struct, so kama's would be a second, incompatible type.
...
- **The host header.** A build of a program that exposes anything also writes **`<project>.h`** beside its
  output (a loose build names it for the output, less a library's `lib`): the prototype of every exposed
  function and every type they name, generated from the same tables as the definitions, so a host never
  hand-writes a declaration that can drift from the kama signature. The project's own `csources` get it on
  their include path. It is plain C (clean C11 and C++17 under `-Wall -Wextra -Werror`, with an `extern "C"`
  ...
  (a loose build writes beside its source, where `shim.h` may be the user's own). <!-- test: expose_host_header -->
- **`type expose value` — a layout kama owns.** A host header can only name a kama value opaquely unless kama
  promises its layout, and the marker is that promise. The header then publishes the whole struct from the kama
  declaration — the pair of `type extern value`, whose layout a C header states:

  ```kama
  @linkName("FrameStats")                       // the host's name for it; otherwise module-qualified
  type expose value Stats {
      public int32 frames;
      public float32 dt;
      public ctor of(int32 frames, float32 dt) { this.frames = frames; this.dt = dt; }
  }
  expose fn Stats frameStats() { return Stats.of(frames: 60, dt: 0.016); }
  ```
  ```c
  typedef struct FrameStats { int32_t frames; float dt; } FrameStats;   /* in the generated header */
  FrameStats game_frameStats(void);
  ```
...
- **Free functions — and one type form.** `expose` is not a member modifier; on a method or field it is <!-- xfail: expose_on_method -->
  rejected. On a type it is spelled `type expose value` (below), directly after `type`.
...
with no such member, a `K::ZZZ` with no such variant, a type where a value is expected, and likewise a
member that does not exist (`v.zz` on a type with no such field, `a.foo` on an `int32`) — is kama's own
error: it is rejected by `kama check`, in kama's vocabulary, before any C compiler runs <!-- xfail: unresolved_name, unresolved_qualified_name, enum_unknown_variant_scope, enum_type_as_value, bare_variant_name, no_such_field, field_on_primitive, method_as_field -->
...
- **Library (wire backends, swappable):** the `Serializer` / `Deserializer` contracts (`writeInt32`/`readInt32`/…,
  ...
  **The object-graph walker is the compiler's**, and it emits nothing but those same tokens: the id table is
  framed with `beginArray(count)` and a pointer written as `writeU64`, so a backend that carries values
  carries graphs — positional and numbered included — with no graph member on either contract and no
...
- **Compiler (opt-in reflection + the walk):** the per-type field walk — the by-value
  `serialize`/`deserialize` for a tree type; for a node type, that pair IS the graph walker, over internal
  C helpers (visit/write/read/re-wire its edges) and a pure-C id table. Zero-cost — emitted **only** for
  `@generate` types, and only the half a type needs. A node carries no extra public surface: no wrapper
  type, no adapter members, no runtime type registry.
...
- **By-value (tree):** a `value` type (owns nothing) or a `resource` reaching no `Shared`/`Weak` (strings,
  ...
  `Optional<Owned<X>>`, like every other field. The compiler recognises the handle **by identity** and walks
  through it; the triad carries no `serialize` of its own, because a method on a handle would shadow the
  pointee's through auto-deref. One consequence worth having: a field may change between `Owned<X>` and a
  bare `X` and still read data written by the other. <!-- test: ser_owned_tree -->
...
- **Graph (heap):** anything reaching a `Shared`/`Weak` is a graph — it can alias, cycle, or hold a
  ...
  **There is no wrapper type: what you hand a backend IS the root.** `serializeJsonBuffer(v: root)` on the
  way out, `deserializeJsonBuffer::<Shared<Node>>` on the way in — the read gives back a **handle**, because
  a cycle cannot be returned by value. The wire is the id-table envelope
  `{"root":1,"objects":[{"id":1,"type":"Node","value":{…}},…]}`, every frame an ordinary token (the table an
