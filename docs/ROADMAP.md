...
| 8 | **Field addressing — every `@field` carries a name AND an id.** `Serializer.field(name:, id:)`, `Deserializer.field() -> FieldKey` (`Name` or `Id`), `beginObject(count:)`, `@field(id:)` defaulting to the declaration index, `@deprecated` = read when present, never written. One derived body per type; the caller's backend decides what lands on the wire, so named, numbered and positional streams all work with no query and no generic contract (a type cannot conform to `Serializable<K>` twice — no overloading). A source break for backend implementors: two in-tree, `Map`/`SortedMap` entries, three fixtures — so before the tag | M | [§4](ROADMAP_DETAIL.md#s4) |
| 9 | **Positional binary backend** — the agreed-shape serializer beside KBIN: no names, no per-value tags, `field()` answers `Id(position)`, a shape mismatch is `Err`. Library only once the row above lands (usable before it with a hand-written `deserialize`). The six-field frame KBIN puts at 91 bytes lands at 24 + framing | S | [§4](ROADMAP_DETAIL.md#s4) |
| 10 | **Numbered binary backend** — protobuf-shaped: the id on the wire with a wire type, so unknown fields skip and a stream survives reordering. The third addressing; needs nothing further from the contract | S | [§4](ROADMAP_DETAIL.md#s4) |
