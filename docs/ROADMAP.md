...
| id | item | size | detail |
|---|---|---|---|
| KR-66 | **A program cannot reach its `@globalAllocator` instance** — the pool is one C global only the funnel calls, so kama code cannot read its live count or high-water mark, and a module `static` cannot hold them either (per-isolate). Scheduled 2026-09-17; recommended shape: a prelude intrinsic returning `ref Pool` with the compiler checking `Pool` is the declared type — spelled `globalHeap::<Pool>()`, since kama's turbofish is `::<T>`. ⚠️ Probed 2026-09-20: the escape checker REFUSES a `ref T` result that borrows neither `this` nor a `ref` parameter, so the shape as written is rejected today and the row must first decide how the intrinsic is exempted; see the detail | S? | [§5](ROADMAP_DETAIL.md#s5) |
