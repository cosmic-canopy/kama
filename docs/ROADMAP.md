...
| KR-53 | **`type extern value` carries ctors and methods** — an extern type refuses every runtime member (*"nothing would emit the body"*, which is not true: a member is a kama function, not part of the struct), so behaviour on a C-owned layout needs a wrapper type and a conversion at every boundary. Since `0.9.327` made `type extern value` the ONLY by-value crossing type, that wrapper tax lands on every FFI struct. Open: its construction — ctors only (a ctor must assign every field) vs keeping by-name aggregate init, whose real job is partial init with the rest zeroed (a 10-field C descriptor, 3 set) | M | [§2](ROADMAP_DETAIL.md#s2) |
