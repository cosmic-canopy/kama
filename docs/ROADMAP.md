...
| 2 | **`try new` is concrete-and-bare-only** — no interface handle (`Optional<Owned<Contract>>`), and no placement form: ⚠️ re-probed 2026-08-28, `try new(allocator: a)` has **no grammar rule**, so the emitter guard for it is dead code | S+ | asset loading; and `try new` is the ONE non-panic construction entry, so an arena or an interface element cannot use it | [§2](ROADMAP_DETAIL.md#s2) |
