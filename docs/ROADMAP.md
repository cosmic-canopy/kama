...
|---|---|---|---|---|
| 1 | **The module system — identity, visibility, and the C symbol** — `namespace` and the filesystem path are two ways to name a thing and nothing keeps them agreeing; five SPEC claims turn out unenforced. Replaces the former *C keyword collisions* and *positional `_F<n>` symbols* rows, which are downstream of it | XL | source-breaking, so it lands before the tag or waits for 2.0; `int32 switch = 3;` emits invalid C and `--keep-c` is not reproducible, which is what the README's "drops into an existing C codebase" rests on | [design/module-system.md](design/module-system.md) |
