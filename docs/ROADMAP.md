...
| KR-89 | **Use-after-move is not diagnosed for a `string` given into a DECLARATION INITIALIZER** — `string b = give a;` then reading `a` builds clean and yields `""`, silently, though the identical shape on every other owning type is refused. kama states the guarantee on its front page. Probed to ONE cell: the read side is complete and the emitter already nulls the source — it just never marks the local moved | S | [§2](ROADMAP_DETAIL.md#s2) |
