...
|---|---|---|---|---|
| 1 | **Go-to-definition on a compiler built-in lands nowhere** — `string`, `isize`, `int32` are registered in C++, so the LSP has no location to return. Take the **prelude** with it: `Optional`/`Result`/`Ordering` are real declarations in a real file that lost their path when embedded, and this repo's own `prelude/` wants special-casing to the worktree | M | every kama program uses them, so it is the most-hit navigation in the language | [§1](ROADMAP_DETAIL.md#s1) |
