...
| 4 | **A `foreach`/`match` binder named like a FIELD is accepted where a declaration is refused** — a CONSISTENCY question, not a defect: probed on `0.9.252`, six shapes, every one behaves correctly (the binder wins, the field is untouched). Needs a verdict — schedule the diagnostic, declare the exemption deliberate, or document the asymmetry | S | [§2](ROADMAP_DETAIL.md#s2) |
| 5 | **23 `tests/xfail` fixtures use the retired class-named constructor** and pass only because their `.msg` still appears among 2–13 errors; rewrite each to `ctor make` + the brace import and confirm ONE error. Tests only | S | [§1](ROADMAP_DETAIL.md#s1) |
| 6 | **`std::time` calendar** — civil-from-days, ISO-8601 format/parse, no zone database; whole or not at all | — | [§1](ROADMAP_DETAIL.md#s1) |
