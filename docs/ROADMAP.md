...
| id | item | size | detail |
|---|---|---|---|
| KR-48 | **Verify the allocation funnel on Windows** — the funnel shipped (`0.9.366`–`0.9.369`): every allocation in emitted C, the runtime headers, the prelude and the stdlib goes through `kama_alloc(n, align)`/`kama_free(p, n, align)`, held by `check-alloc-funnel.sh` and proven layout-exact by the san leg. It was built and tested on Linux + wasm only, and the Windows branch of `kama_os.h` has never been compiled since (no SDK headers on that box): `kama__wide`/`kama__wpath`/`kama__wfree` on sized blocks, `kama_diropen`, the Winsock poller, `kama_argv_*`/`kama_envp_build`, `kama__win_cmdline`/`proc_spawn`, `kama_capture2`'s thread drain, and `_aligned_malloc`/`_aligned_free` beyond the fundamental alignment. `./dev test` then `./dev check` on the Windows box closes it (no containers there; san/wasm are the other boxes' job). Allocation campaign step 2 | S | [§5](ROADMAP_DETAIL.md#s5) |
| KR-49 | **A replaceable global allocator** — `kama_alloc`/`kama_free` delegate to a global allocator that defaults to `malloc`/`free` and that a program replaces. The surface is DECIDED: a **declaration** (`@globalAllocator type value Pool implements Allocator`), because it has a body `--no-heap` can check, where a weak link-time symbol could only be trusted. Wants the maintainer's call on what the flag promises first — never allocates, or never reaches the SYSTEM heap (a checked pool over program-owned storage allowed). `Shared.adopt`'s hard-coded control block and the `SortedMap` root move onto `A` in the same step. Allocation campaign step 3 | M | [§5](ROADMAP_DETAIL.md#s5) |
| KR-50 | **Allocator-aware errors** — boxing an error into `Owned<Error>` calls `malloc` directly instead of drawing from an allocator; route it (and primitive boxing) through the box's `A`, so a replaced global allocator covers every fallible API. The PER-CALL error allocator (a contract change to serde, source-breaking) is likely unnecessary once KR-48/49 land — errors then follow the replacement — so decide it last, on evidence. Allocation campaign step 4 | M | [§5](ROADMAP_DETAIL.md#s5) |
