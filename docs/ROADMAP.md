...
| KR-32 | **The debugger shows emitted-C names and values** — stepping and breakpoints are already kama-source-level, but a `string` inspects as `kama_string {data,len,cap}`, an `Optional` as its tagged union, a type as `_Ffile__V`, a frame as `_Ffile__V__make`, and an enum value as `_Ffile__Mode_CopyFile`. Three parts: LLDB summaries for the values; synthetic children so a struct's fields read as kama names; and a name layer in the VS Code extension between it and CodeLLDB for LOCALS, the CALL STACK and watch expressions, which no formatter can reach. The first two are independent; the third needs a demangle surface, which the C-names work left for this row to choose (it ships a reversible mangling, not a map). Moved out of LATER 2026-09-17: the C-names work (shipped `0.9.398`, SPEC § *C names*) prefixes locals too, so this stops being polish | L? | [§10](ROADMAP_DETAIL.md#s10) |
| KR-68 | **Every module's C preprocesses the whole OS header set** — one generated `<name>.gen.h` carries every `extern` header, so importing `std::fs` anywhere puts `<windows.h>` (21,748 macros) in front of EVERY module's C. Preprocessing `kama_os.h` measured ~263 ms against ~121 ms for `kama_runtime.h` alone on the Windows VM (ratio only — that box is QEMU + x64 emulation). Moving the seam behind plain prototypes, with bodies in one TU, would cut per-TU preprocessing on every platform (GOALS #2) and shrink the macro surface; it costs inlining of the thin syscall wrappers, so it is measured on a NATIVE box first. Not a fix for the name-collision class (shipped `0.9.398`, SPEC § *C names*) — a user's own `extern` header bleeds either way | M? | [§9](ROADMAP_DETAIL.md#s9) |
| KR-69 | **A misspelled triple-component flag in a gate is silently inactive** — `@compileFor(OS_WINODWS)` (and a `csources` entry's `compileFor`) builds and passes `kama check`: any `OS_`/`ARCH_`/`ABI_` name validates because triples are open. Measured `0.9.399`: under a manifest `@compileFor(WINODWS)` IS refused while `@compileFor(OS_WINODWS)` beside it passes; and the set of gates `kama check` covers is HOST-DEPENDENT — a type error behind `ARCH_AARCH64` reports OK on an x86_64 box. Recommended: lift SPEC's stub-code allowance — a gate must be activatable by a target kama KNOWS (a host-independent catalog, a declared target, or this build's triple), else it is an error — which needs no spelling table and also catches a valid-but-wrong component. The table is the fallback if the allowance stays | M | [§2](ROADMAP_DETAIL.md#s2) |
| KR-70 | **A dependency cannot ship a prebuilt archive** — a relative `-L` in a dependency is refused (correctly) and `link` names no directory, so a package wrapping a cmake-built library leaves its consumer to link it. **Proposed verdict: NON-GOAL** (awaiting the maintainer's word) — a source-first ecosystem with a guaranteed C compiler does not carry per-target binaries; what answers the need is `csources`, or `link` in the package with the ROOT saying where the library is | ? | [§2](ROADMAP_DETAIL.md#s2) |
