...
| 7 | **No way to ask for reproducible floating point.** The driver emits no `-ffp-contract`, so clang's default `on` contracts `a*b + c` into an FMA and a cross-target program cannot say it needs bit-reproducibility except through raw per-target `cflags` — which do not propagate to a dependency (the dependency-flags row above). Measured by the first external project: 10 of 64 random triples differ on aarch64-macos, 0 with `-ffp-contract=off`; arm64 has FMA, wasm32 MVP and baseline SSE2 do not | S | [§2](ROADMAP_DETAIL.md#s2) |
