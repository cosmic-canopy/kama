...
| 9 | **`drop(value: x)` on an RAII-owned local DOUBLE-FREES** — the emitted C calls the destructor twice, once for the explicit drop and once at scope exit, and the program SIGSEGVs. Not collection-specific: a plain `resource` local, an `Owned<T>` box and a `Shared<T>` handle all do it (`Shared` underflows its refcount, which is worse — it frees a pointee other handles still hold). ⚠️ It is SPEC's own documented spelling; what SPEC describes is the raw-pointer case, where no RAII collides. ⚠️ PRE-EXISTING (traps at `0.9.282` too). ⚠️ The silent sibling: `drop(value: give x)` emits `(void)0` — safe, but the explicit drop does NOTHING with no diagnostic, so the spelling that works quietly does nothing and the documented one crashes. Needs a decision — refuse `drop()` on an RAII-owned local, or suppress the scope dtor when one is seen (flow-sensitive). ⚠️ **Its POSITION here is my judgement — it is the most severe thing open, so I put it first; move it if you disagree** | M | [§2](ROADMAP_DETAIL.md#s2) |
