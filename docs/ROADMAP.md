...
| KR-15 | **Compiler defects found building AND reviewing the serde split** — ⚠️ **RE-PROBED 2026-09-12 against `0.9.309`: all six still reproduce, three are WIDER than recorded, and running repros for each are kept at `.scratch/next-session/` (with a README table).** (1) The stdlib `Weak<Contract>` does not monomorphize (`weak.kama:36`, *"cannot tell which `Shared` to construct"*) — ⚠️ **and it is ORDER-DEPENDENT**: one unrelated, never-called `fn int32 touch(Weak<Shape> w)` makes the same program build, which is exactly why `tests/shared_iface.kama` (it has `fn Weak<Shape> orphan()`) passes and hides it. (2) A FUNCTION-CALL temporary passed to a contract-typed parameter emits `&` of an rvalue — ⚠️ **`kama check` passes, clang refuses**; a `T.of(…)` compound literal passes. (3) A generic `ctor` in ARGUMENT position does not infer — ⚠️ **not even against a CONCRETE `Box<int32>` parameter**; (1) and (3) likely share `dotOnTypeInstance`, which reads only a turbofish or `_variantTargetType`. (4) A `when [P: Source<int32>]` gate never holds (*"`Wrap<P>` has no method `pull`"*) — `resolveWhenBound` drops the bound's generic arguments. (5) An enum PAYLOAD graph edge reports *"not `@generate`"* about a type that is — ⚠️ **wider: an enum is never a graph node and a payload `Shared` is not walked as an edge at all**, so whether it is supported or refused cleanly wants a ruling before code. (6) A read-side refusal on an enum payload says ``field `__p_w` `` where the write side says `` `w` ``. Each wants a fixture or an `xfail` | L | [§4](ROADMAP_DETAIL.md#s4) |
