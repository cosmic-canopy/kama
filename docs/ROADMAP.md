...
| 8 | **Restricted-private contract members** — a contract may declare a member non-public, with a `friend` grant naming who may reach it; an implementer then matches that visibility instead of being forced `public`. ⚠️ **Two accepted-and-inert surfaces are the starting point, both MEASURED:** `private` on a contract member parses today and is **silently dropped** (a call through a contract value reaches it), and `friend` on a contract was refused at `0.9.266` for being inert in the same way. So step 1 is proving the forced-`public` rule is only load-bearing because the call-site half was never built; step 2 restores `friend` on contracts, fully enforced, failing closed on both old holes (`tests/xfail/friend_on_contract` and its doc claim change in the same commit). `friend X[members]` already grants fields, methods and ctors alike. ✅ **Both spellings already PARSE, so this is emitter-only** — no grammar, no tree-sitter, no Zed pin. `InterfaceMethod` carries no visibility field yet; the call-site hook is `emitInterfaceDispatch`, beside the per-member `@noheap` gate it already runs there. Unblocks row 9's container seam: `private` with no grant is "only the compiler may call it", with no new keyword | M | [§4](ROADMAP_DETAIL.md#s4) |
| 9 | ⛔ **BLOCKED on the serde architecture review (2026-09-09)** — the container work was stopped: the layering that produced the adapters is itself under review (7 public synthesized members on every node type, `ObjectGraph<T>` call-site ceremony, `43dd3dd`'s method on the method-free triad). Settle the architecture first; the gaps below are the consumers of whatever it decides. **Serialization follow-ups — the GRAPH GAPS first** — a collection of edges (`DynamicArray<Shared<Node>>`: a node with N children, the canonical graph shape, refused today with a message that says so) and an enum payload holding one; a fat `Owned<Contract>` field. Then deserialize breadth (`FixedArray`/`InlineArray` read, a bare buffer of an intrinsic), yaml/xml back ends, a `Format` contract so a back end's entry points are checked rather than a convention | — | [§4](ROADMAP_DETAIL.md#s4) |
