...
| 9 | **Serde: containers of pointers** — a `DynamicArray<Shared<Node>>` / `<Owned<X>>` field. The architecture is settled and shipped (`0.9.270`–`0.9.274`): the triad is method-free, the walker is the compiler's again over the token envelope, reach walks a generic instance's type arguments, and all three handles satisfy the serde bounds through a derived arm — so every stdlib container's existing `when [T: Serializable]` gate is already correct. ✅ **The language gate is GONE** (`0.9.275`): an element access is a nameable receiver for a primitive element too, so every container writes through `this[i].serialize(w)` and no `const ref T` names a smart pointer. The WRITE half works. What remains is the READ half, and it is two different questions — `Owned<X>` needs a `deserialize` that is not a method on the triad, and a container of EDGES needs the walker's id table, which a `serialize` receiving only a `Serializer` cannot reach. Both pinned (`tests/xfail/ser_coll_of_owned`, `graph_coll_of_shared` — when the language rule lifts, the second one's message changes, and that is the signal) | M | [§4](ROADMAP_DETAIL.md#s4) |
