...
| 4 | **A `foreach`/`match` binder named like a FIELD** — the field-shadow ban is declarator-only, so a binder carrying a field's name resolves the bare name to the field wherever the local tables miss (reached one case more often since `0.9.248`); extend "shadows a field" to both binders, an `xfail` each | S | [§2](ROADMAP_DETAIL.md#s2) |
