...
| # | item | size | detail |
|---|---|---|---|
| 1 | **`@kama/sodium` — the first official package, and the package-naming decision it carries** — the `@kama` scope is the mark of an official package and its channel (npm/JSR/NuGet/Go's answer; the mechanism already ships), `@kama`/`@std` get reserved when the hosted registry exists, and `kama seed --license mit` lands first so the bundle a package publishes with is complete. The binding itself (libsodium: `secretbox`/`box`/`sign`/`randombytes`) lives OUTSIDE this repo as a peer, seeded with the recommended AI bundle | M | [§10](ROADMAP_DETAIL.md#s10) |
| 2 | **Generic inference types an unsuffixed literal as `int32` before a typed sibling can bind `T`, and cannot bind `T` from a receiver call** — `range(rng: g, lo: 0, hi: xs.length())` does not compile; defer the literal, and let a method call on a typed local answer. The reason `std::random` has `below(n:)` beside `range` | S | [§2](ROADMAP_DETAIL.md#s2) |
| 3 | **Generic `min`/`max`/`clamp` over `Comparable<T>`** — `std::math` has no INTEGER `min`/`max`/`clamp`; `minI32`…/`minf`… are the last suffixed ladders. The first row of a stdlib M3 | S | [§1](ROADMAP_DETAIL.md#s1) |
| 4 | **A module's file-private names are one namespace** — two files in one module cannot both declare a non-exported `fn helper` ("duplicate function"), and a file-private `type T` in one file shadows the other file's own `T` at lookup, which is then refused as "not exported". The compiler's own message says visibility is per FILE; the tables behind it are per MODULE. Found writing the first package's tests | S | [§2](ROADMAP_DETAIL.md#s2) |
| 5 | **No const raw pointer** — `addr(of:)` on a const receiver is refused by design and `dataPtr()` is non-const, so a read-only buffer cannot reach C through a `const ref`; every FFI wrapper takes `ref`. Wants an `UnsafePtr<const T>`-shaped answer or a const `dataPtr`, decided once | M | [§2](ROADMAP_DETAIL.md#s2) |
| 6 | **`--release` with `csources` prints `-Wl,-dead_strip: 'linker' input unused` once per C translation unit** — a link-time flag reaches the per-TU compile lines. Cosmetic, and the first thing a package consumer sees | S | [§9](ROADMAP_DETAIL.md#s9) |
| 7 | **`kama seed --kind library` should seed the package half of the agent guidance** — publishing, `csources`/`cincludes`, vendoring a C library, `tests/` as one program, `--license`. `kama agents install` has no kind to key on, so the addendum needs a home that survives a re-install | S | [§10](ROADMAP_DETAIL.md#s10) |
