...
| # | item | size | detail |
|---|---|---|---|
| 1 | **An unresolved NAME or MEMBER in expression position is diagnosed by clang, not by kama.** `fn int32 main() { return Nope; }` — an identifier that exists nowhere — passes `kama check` clean, and `kama build` reports it as `use of undeclared identifier 'Nope'`; `a.foo` on an `int32` local reports "member reference base type 'int32_t' (aka 'int') is not a structure or union". The POSITION is right (`#line` puts it on the kama line); the vocabulary is C, about C types, in a file the reader wrote in kama — and `kama check`'s own usage text promises "name resolution". Dot-on-type is the sharpest case because the sibling diagnostics already exist and are good: `V::make(…)` and `V.f()` are both refused with advice naming the right operator, while `K.A` for an enum variant (SPEC: `::` is the associated-item operator) emits `(K).A` and reaches clang, and `K.ZZZ` — no such variant — does too. Found while fixing the field-type read-site bug (`0.9.176`), which had MASKED it: the spurious import error fired first, so no build ever got far enough to meet it | ? | [§2](ROADMAP_DETAIL.md#s2) |
| 2 | **`exprClass` is unreliable inside a generic instantiation**, so the class-identity rule stops at the boundary of one. In `Owned<T, A>.adoptIn` the FIELD answers the substituted `BumpAllocator` while the `A allocator` PARAMETER still answers the default `GlobalAllocator`. Same family as the generic-scan drift. Until it is fixed, `Mat4 m = someVec4;` is caught everywhere EXCEPT inside a generic body | M | [§2](ROADMAP_DETAIL.md#s2) |
