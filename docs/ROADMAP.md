...
| 9 | **Serde: containers of pointers** — a `DynamicArray<Shared<Node>>` / `<Owned<X>>` field. The architecture is settled and shipped (`0.9.270`–`0.9.274`): the triad is method-free, the walker is the compiler's again over the token envelope, reach walks a generic instance's type arguments, and all three handles satisfy the serde bounds through a derived arm — so every stdlib container's existing `when [T: Serializable]` gate is already correct. ⚠️ **One LANGUAGE rule gates both shapes and fires first** (row 22): the container writes elements through a `const ref T` helper, because `this[i].serialize(w)` does not resolve for a generic element, and a `ref` parameter may not name a smart pointer. Behind it sits the serde half: a container's `serialize` receives only a `Serializer`, so an element write has nowhere to get the walker's id table from. Both pinned (`tests/xfail/ser_coll_of_owned`, `graph_coll_of_shared` — when the language rule lifts, the second one's message changes, and that is the signal) | M | [§4](ROADMAP_DETAIL.md#s4) |
...
| 33 | **`const ref T` may not name a smart pointer, and it gates container serde** — a `ref`/`const ref` parameter is refused when `T` binds to `Owned`/`Shared`/`Weak`, even for a READ-ONLY borrow of the handle. That is what stops `DynamicArray<Owned<X>>` and `<Shared<X>>` from serializing (row 9), and it fires **from stdlib source** (`dynamic_array.kama:259`), never naming the user's file. ⚠️ The helper is load-bearing, verified by deleting it: `this[i].serialize(w)` does not resolve for a generic element, which is why the `const ref T` hop exists at all. So there are two fixes and they are independent — relax the rule for a read-only borrow of a handle, or make an element receiver nameable so the hop is unnecessary. Decide which before touching row 9 | S | [§4](ROADMAP_DETAIL.md#s4) |
