...
| KR-20 | **`csources` compiles C, not C++** — a `.cpp`/`.cc`/`.cxx`/`.m`/`.mm` entry is refused by name. MEASURED 2026-09-17: the C++ runtime follows the DRIVER (`clang++`/`g++`/`zig c++`/`em++`), so no per-target runtime table is needed; `-std=` is not positional, so C and C++ cannot share one invocation (group by language to keep zig's cache); per-target `csources` and a home for C++-only flags need rulings first. ⚠️ Carries a LIVE BUG: zig/wasm builds compile kama's own C as `gnu11` whenever a `csources` entry exists; and unknown keys in a target arm are silently skipped. The consumer's Makefile retires to its cmake half | L | [§2](ROADMAP_DETAIL.md#s2) |
