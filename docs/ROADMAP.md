...
| 8 | **Serialization follow-ups** — deserialize breadth (`FixedArray`/`InlineArray` read, a bare `encode`/`decode` of an intrinsic), yaml/xml back ends, a `Format` contract so a back end's entry points are checked rather than a convention | — | [§4](ROADMAP_DETAIL.md#s4) |
