...
| 4 | **A kama reserved word cannot name a `type extern value` field, and there is no raw-identifier escape** — `webgpu.h` puts `type` in four structs, and an extern struct's fields are assigned BY NAME, so unlike a function it cannot be wrapped or renamed. ⚠️ **Sharpened by the consumer's M1-a, and it is worse than "imprecise": the omitted field leaves 0, which IS `WGPUBufferBindingType_BindingNotUsed` / `WGPUSamplerBindingType_BindingNotUsed`, so an explicitly built layout entry for a uniform buffer or a sampler is INERT, not under-specified.** `WGPUBindGroupLayoutEntry` embeds both by value, so `deviceCreateBindGroupLayout` cannot express a uniform + sampler + texture group **at all**; the only escape is `renderPipelineGetBindGroupLayout`, which is PER PIPELINE, so no layout can be shared between pipelines. Their MSDF text stage is the trigger. Wants a spelling (`@cname("type")`, `` `type` ``, or similar) | M | [§2](ROADMAP_DETAIL.md#s2) |
