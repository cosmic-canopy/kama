...
| KR-61 | **`Allocator.allocate` has no alignment** — a block is sized but never aligned, so an allocator can only guess: `BumpAllocator` rounds to 8, and a type holding a `Simd<float32>#(4)` (`alignof` 16) lands at an address ≡ 8 mod 16 in an arena — misaligned storage, undefined behaviour an SSE load can fault on. Decide the contract (an `align` parameter, as Rust's `Layout` and Zig's `alignedAlloc` carry) before KR-49 freezes the global allocator's shape | M? | [§5](ROADMAP_DETAIL.md#s5) |
