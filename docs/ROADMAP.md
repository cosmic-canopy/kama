...
| KR-50 | **Allocator-aware errors** — boxing an error into `Owned<Error>` calls `malloc` directly instead of drawing from an allocator; route it (and primitive boxing) through the box's `A`, so a replaced global allocator covers every fallible API. The PER-CALL error allocator (a contract change to serde, source-breaking) is likely unnecessary once KR-49 lands on the shipped funnel — errors then follow the replacement — so decide it last, on evidence. Allocation campaign step 4 | M | [§5](ROADMAP_DETAIL.md#s5) |
