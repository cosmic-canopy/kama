...
| # | item | size | detail |
|---|---|---|---|
| 1 | **A module's file-private names are one namespace** — two files in one module cannot both declare a non-exported `fn helper` ("duplicate function"), and a file-private `type T` in one file shadows the other file's own `T` at lookup, which is then refused as "not exported". The compiler's own message says visibility is per FILE; the tables behind it are per MODULE. Found writing the first package's tests | S | [§2](ROADMAP_DETAIL.md#s2) |
| 2 | **No const raw pointer** — `addr(of:)` on a const receiver is refused by design and `dataPtr()` is non-const, so a read-only buffer cannot reach C through a `const ref`; every FFI wrapper takes `ref`. Wants an `UnsafePtr<const T>`-shaped answer or a const `dataPtr`, decided once | M | [§2](ROADMAP_DETAIL.md#s2) |
| 3 | **`--release` with `csources` prints `-Wl,-dead_strip: 'linker' input unused` once per C translation unit** — a link-time flag reaches the per-TU compile lines. Cosmetic, and the first thing a package consumer sees | S | [§9](ROADMAP_DETAIL.md#s9) |
| 4 | **`kama seed --kind library` should seed the package half of the agent guidance** — publishing, `csources`/`cincludes`, vendoring a C library, `tests/` as one program, `--license`. `kama agents install` has no kind to key on, so the addendum needs a home that survives a re-install | S | [§10](ROADMAP_DETAIL.md#s10) |
| 5 | **No capturing closures** — a handler must be a `type resource` carrying its captures plus a `BindableFunctionPtr`, which is the shape a UI event table wants least. The first consumer works around it and says it will bite properly at their UI milestone. ⚠️ Rowed here for the first time: it was cited as already being on this list and was not | — | [§2](ROADMAP_DETAIL.md#s2) |
