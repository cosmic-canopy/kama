...
| KR-57 | **A binding may take the name of a function in scope** — **decided 2026-09-22: resolved by KR-87** (a binding may not take the name of any function in scope; once the floor is imported like every module, "in scope" means declared or imported, with no prelude exception). Implement in the same arc as KR-87, not before — a rule written against today's implicit floor would be replaced. Corpus cost measured: 7 sites (6 named like a user/stdlib function, incl. std::net::udp's public `interfaceIndex:` label; 1 `args` parameter in std::process) | S | [§2](ROADMAP_DETAIL.md#s2) |
