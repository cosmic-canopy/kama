...
| 8 | **Serialization follow-ups** — deserialize breadth (`FixedArray`/`InlineArray` read, a bare `serializeJsonBuffer`/`deserializeJsonBuffer` of an intrinsic), yaml/xml back ends, a `Format` contract so a back end's entry points are checked rather than a convention | — | [§4](ROADMAP_DETAIL.md#s4) |
| 9 | **Serde: graph moves to the library, contracts keep only the public token protocol** — an XL design item, and the one that reshapes the rest. Today the compiler emits the whole graph algorithm (id table, dedup, two-pass wire, cycles, heap shells) and `Serializer`/`Deserializer` carry 8 intrinsic-only members no hand-written code has any reason to call. Decided: the graph is a LIBRARY concern; the derive keeps emitting only per-type ADAPTERS (opt-in reflection, unchanged — no universal metadata), and the algorithm moves out. That removes the members, so it also removes the need for the protected-base machinery the split first seemed to need. Prerequisite for the KNUM graph row and for answering "how does a user know graph is supported". Every measured fact is in the detail — do not re-derive | XL | [§4](ROADMAP_DETAIL.md#s4) |
| 10 | **Serde naming pass** — the type names now say `{Addressing}{Medium}Serializer` and the binary module's entry points name their addressing, but the MODULE names still mix axes: `json` is a format, `binary` a medium. Renaming is a source break, so it belongs before the tag or not at all. Also folds in the general `@deprecated` declaration marker (a use-site warning), which must EXTEND the field meaning that shipped, not redefine it | S | [§4](ROADMAP_DETAIL.md#s4) |
