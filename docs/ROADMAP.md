...
| id | item | size | detail |
|---|---|---|---|
| KR-48 | **Verify the allocation funnel on Windows** — the funnel shipped (`0.9.366`–`0.9.369`): every allocation in emitted C, the runtime headers, the prelude and the stdlib goes through `kama_alloc(n, align)`/`kama_free(p, n, align)`, held by `check-alloc-funnel.sh` and proven layout-exact by the san leg. It was built and tested on Linux + wasm only, and the Windows branch of `kama_os.h` has never been compiled since (no SDK headers on that box): `kama__wide`/`kama__wpath`/`kama__wfree` on sized blocks, `kama_diropen`, the Winsock poller, `kama_argv_*`/`kama_envp_build`, `kama__win_cmdline`/`proc_spawn`, `kama_capture2`'s thread drain, and `_aligned_malloc`/`_aligned_free` beyond the fundamental alignment. `./dev matrix` there closes it. Allocation campaign step 2 | S | [§5](ROADMAP_DETAIL.md#s5) |
