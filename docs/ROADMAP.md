...
| KR-41 | **A `type extern value` accepts member functions it never emits, and `kama check` passes** — a named `ctor` or a method on a C-layout type is accepted, the call is emitted, no definition is, and the C compiler reports the undefined function. A rejection arriving from clang, which is the class this repo hunts; the design intent already says these types are POD, so the fix is a refusal | S | [§2](ROADMAP_DETAIL.md#s2) |
