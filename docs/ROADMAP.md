...
| 9 | **Serde: a hand-written type as a graph NODE** — the last piece of "one rule for every `Serializable`". A hand-written `Serializable` can now be a graph PARTICIPANT (it holds edges, as a root or as a field) but not a NODE (a thing an edge points AT): its pointees must still be `@generate`, which is why a hand-written CYCLE is a clean compile error rather than working. The write side is mechanical — `K__visitEdges`/`K__writeNode` are the twin, run against the discard sink and against the writer. ⚠️ The READ side has a real conflict to settle first: a node's root spelling is `Shared<T>` and its `deserialize` must return `Result<Shared<This>, …>`, which `computeGraphNodeTypes` REWRITES for a derive — but a hand-written body DECLARED `Result<This, …>` and its `return` constructs a `This`. So the handle root needs a synthesized second entry point rather than a retarget, and `X__serialize` has to fork on whether `X` is a node (root carries an id) or only a participant (root carries its value). ~17 `genSerialize`/`genDeserialize` filter sites gate the node helpers and would take a `graphNodeWrites`/`graphNodeReads` pair | M | [§4](ROADMAP_DETAIL.md#s4) |
| 10 | **`copy` on a raw-pointer index silently drops the copy marker** — `Optional::Some(value: copy this.vals[i])` where `vals` is an `UnsafePtr<V>` emits `.value = (self->vals)[i]`, a bitwise ALIAS of a buffer the owner still holds; the caller's dtor then frees it out from under the owner. The SAME expression as a direct call argument does emit a real `__copy`, so it is the variant-payload construction that loses it, and there is no diagnostic. `map.kama` and `sorted_map.kama` work around it with `cloneVal`/`cloneKey(const ref V)` helpers — and a `const ref` may not name a smart pointer, so **`Map<K, Shared<V>>` does not compile at all**, serde or not. ⚠️ MEASURED that the helpers are still load-bearing after `0.9.275` (stdlib as-is returns 2, inlined traps at 133), so this is not a leftover to delete — the marker has to be fixed first, and the helpers then go the way the seven `serElem` ones did. Blocks the `Map`/`SortedMap` case of the serde graph work; `Set`/`SortedSet` of edges is refused by bound and is a non-issue | M | [§2](ROADMAP_DETAIL.md#s2) |
| 11 | **Serde naming pass** — the type names now say `{Addressing}{Medium}Serializer` and the binary module's entry points name their addressing, but the MODULE names still mix axes: `json` is a format, `binary` a medium. Renaming is a source break, so it belongs before the tag or not at all. Also folds in the general `@deprecated` declaration marker (a use-site warning), which must EXTEND the field meaning that shipped, not redefine it | S | [§4](ROADMAP_DETAIL.md#s4) |
| 12 | **Compiler defects found building AND reviewing the serde split (`0.9.269`)** — the stdlib `Weak<Contract>` does not monomorphize AT ALL (`tryUpgrade`'s `Shared.make` cannot infer over a contract element; no fixture ever used it); a TEMPORARY passed to a contract-typed parameter takes the address of an rvalue in the emitted C (bind a local meanwhile); a generic `ctor` in ARGUMENT position does not infer its type argument from its own arguments. **Added by the 2026-09-09 review, each MEASURED:** a `when [...]` gate naming a FREE-PARAMETER contract with a concrete argument never holds, though the same bound works in a generic fn's and a generic type's parameter list; and four misleading diagnostics — a collection-of-edges field blames the container with advice that cannot be followed, the "collection OF nodes" message fires only for `Owned<Shared<X>>` and calls a smart pointer a collection, an enum payload edge reports "not `@generate`" about a type that IS, and an internal payload name (`__p_next`) leaks into user-facing text. Each fails loudly, not silently; each wants a fixture or an `xfail` | — | [§4](ROADMAP_DETAIL.md#s4) |
