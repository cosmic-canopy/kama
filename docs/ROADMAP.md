...
| KR-18 | **Windows suite wall-clock — the guard-spawn sweep is DONE; what is left is a MEASUREMENT PROBLEM and a different kind of cost.** Three guards rewritten fork-free (`check-doc-claims` 2026-09-11, `check-query` + `check-lsp` 2026-09-12), each proven byte-identical on the happy AND failure paths. Measured alone, idle: `check-lsp` **46 s → 12 s**, `check-query` **147 s → 89 s**. In-phase `check-query` **239.4 s → 167 s** (two runs 1% apart) and `check-lsp` left the slowest-8 entirely. ⚠️ **NO PHASE WALL-CLOCK WIN IS CLAIMED, and the A/B is not merely "still owed" — it was run and came back INCONCLUSIVE**: 408 s → 346 s, but a REPEAT of the identical post-change config gave 268 s, so run-to-run spread (78 s) exceeds the effect (62 s). Anything wall-level here needs interleaved A/B/A/B over several runs, or a quieter box; per-guard numbers are the ones that reproduce. ⚠️ The row's old figures were contaminated by `check-doc-claims` saturating 12 cores — guards are ~2.5-3× cheaper than recorded, while the spawn:expansion ratio is ~336×, HIGHER than the ~200× on record (a spawn is ~155 ms idle, not ~748 ms). ⏳ Left: `check-packages` is now the top guard (~175 s) and is **not** this class — 103 real `kama` runs, like `check-manifest`'s 67 full builds. Their lever is FEWER COMPILER INVOCATIONS per guard, which is unscoped and arguably its own row | S | [§1](ROADMAP_DETAIL.md#s1) |
