...
| KR-69 | **A misspelled triple-component flag in a gate is silently inactive** — `@compileFor(OS_WINODWS)` (and a `csources` entry's `compileFor`) builds and passes `kama check`: any `OS_`/`ARCH_`/`ABI_` name validates because triples are open. Measured `0.9.399`: under a manifest `@compileFor(WINODWS)` IS refused while `@compileFor(OS_WINODWS)` beside it passes — the prefix is a free pass. Briefed with a recommendation: a name in kama's three reserved namespaces is valid when its component is KNOWN (zig's + LLVM's public lists), DECLARED (a manifest target) or ACTIVE (this build's triple) — which keeps SPEC's stub-code ruling (`ARCH_RISCV64` with no target) and refuses `ARCH_AARCH46`; four decisions for the maintainer | M | [§2](ROADMAP_DETAIL.md#s2) |
