...
| 3 | **Audit every "waiting on a consumer" deferral** — kama is a general-purpose systems language: a feature a consumer will need is decided, not deferred until the corpus asks for it. One verdict per item (schedule · genuinely optional · non-goal, with the reason); each scheduled one becomes its own row. Seeded by the const-pointer arc, which was exactly such a deferral | M | [§3](ROADMAP_DETAIL.md#s3) |
| 4 | **`--release` with `csources` prints `-Wl,-dead_strip: 'linker' input unused` once per C translation unit** — a link-time flag reaches the per-TU compile lines. Cosmetic, and the first thing a package consumer sees | S | [§9](ROADMAP_DETAIL.md#s9) |
| 5 | **`kama seed --kind library` should seed the package half of the agent guidance** — publishing, `csources`/`cincludes`, vendoring a C library, `tests/` as one program, `--license`. `kama agents install` has no kind to key on, so the addendum needs a home that survives a re-install | S | [§10](ROADMAP_DETAIL.md#s10) |
