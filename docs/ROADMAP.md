...
| 9 | **A graph whose edges are a KEYED container's values** — `Map<K, Shared<V>>` compiles and round-trips as a container now (`0.9.285`), but the serde graph read half still refuses it. Pass 2 revisits edge elements through `IterableMut<T>`, and a keyed container cannot coherently implement it: `Map`'s `Iterable` yields KEYS while its edges live in VALUES (it has `ValuesIterableMut` instead). The refusal is correct and names the protocol. ⚠️ This is NOT the blocker the `copy`-marker row named: that drop was measured, once fixed (`0.9.283`/`0.9.284`), not to be what stood in the way. Needs a surface decision: prefer `ValuesIterableMut` when present (smallest, but hard-codes one library contract into the walker) or let a container MARK the accessor the walker should use (more machinery, does not privilege `std::collections`). Either way, write down that a keyed container's edges may live in VALUES only — a key cannot be revisited in place without a rehash, so `Map<Shared<K>, V>` is a non-goal, not a "not yet" | M | [§2](ROADMAP_DETAIL.md#s2) |
