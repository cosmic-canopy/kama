...
| KR-55 | **An enum's values are an ABI promise nothing marks or checks** — the generated host header prints a payload-less enum's constants (`Kind_Square = 4`), and inserting a variant renumbers every later one silently; a kama enum mirroring a C enum is an unchecked hand copy. Decide between explicit values at a boundary and a `type expose enum`/`type extern enum` pair, with the bytes, before code | ? | [§2](ROADMAP_DETAIL.md#s2) |
