...
|---|---|---|---|---|
| **1** | **the analysis gap** — kama never type-checks an expression (⑩) · narrowing `cast<int8>(300)` → 44 (⑪) · the test-infra holes | ⑩⑪ | [design/analysis-gap.md](design/analysis-gap.md) | **⑦ is CLOSED** — `foreach` is a window now, so growing a container mid-loop is a compile error, and `mods` is bumped in `growTo` where the buffer actually moves. What is left was **re-probed 2026-08-15, and the description this row used to carry was wrong about the root cause**: ⑩ is not a generics problem. `int32 x = "not an int";` in a plain `main` passes `kama check` clean and fails only at **clang**, against generated C the author never wrote — kama has **no expression type-checker at all**, and the uninstantiated-generic hole (a body with four errors builds clean) is a second, separate defect on top of it. That makes ⑩ a missing SUBSYSTEM, not a missing rule, and the largest correctness item before the tag. ⑪ is two problems: the constant case is mechanical and provable, the runtime case is a design decision (truncate like Rust's `as`, trap like the bounds check, or a second spelling) — **decide it before writing code**. |
