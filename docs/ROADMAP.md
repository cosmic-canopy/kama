...
| id | item | size | detail |
|---|---|---|---|
| KR-63 | **A `type value` may hold a move-only field and silently copy it** — `type value Holder { Atomic<int32> a; }` builds, and `Holder k = h;` makes two cells out of one (measured: `k.a.store(5)` leaves `h.a` at 1), though `Atomic<int32> y = x;` alone is a move. The value check refuses a field that is `destructible` and skips the `moveOnly` it already computes; SPEC says a resource "owns something, **or has identity**", and identity is what was laundered. Refuse a `value` with any move-only field. Prerequisite for KR-49, whose global allocator is exactly such state | S | [§2](ROADMAP_DETAIL.md#s2) |
| KR-49 | **A replaceable global allocator** — `kama_alloc`/`kama_free` delegate to a global allocator that defaults to `malloc`/`free` and that a program replaces with a **declaration** (`@globalAllocator type value Pool implements Allocator`), because a declaration has a body `--no-heap` can check, where a weak link-time symbol could only be trusted. The no-heap leaf moves from `GlobalAllocator` to the funnel's DEFAULT implementation, so a declared pool over program-owned storage makes boxes, containers and error boxes legal under `@noheap` and `--no-heap` alike. **Recommended, awaiting the maintainer's yes:** both spellings keep ONE meaning, *never reaches the system heap* — what they already mean today (design doc, *Still open* item 1). **The instance** is ONE process-global object emitted in the entry TU, the `setPanicHandler`/argv precedent, and its fields follow the sharing-seams rule (`Atomic`, `const`, raw storage); it is a `type resource`, so KR-63 closes first (design doc §3, *RECOMMENDED SHAPE*). `Shared.adopt`'s hard-coded control block and the `SortedMap` root move onto `A` in the same step. Allocation campaign step 3 | M | [§5](ROADMAP_DETAIL.md#s5) |
| KR-50 | **Allocator-aware errors** — error and primitive boxing already draw from the funnel (`0.9.367`), so after KR-49 they follow a declared global allocator with no further change; what is left is to PROVE that (a serde error under `--no-heap` with a declared pool) and to decide the PER-CALL error allocator (a contract change to serde, source-breaking) on that evidence — expected verdict: a non-goal, recorded with its reason. Allocation campaign step 4 | S? | [§5](ROADMAP_DETAIL.md#s5) |
