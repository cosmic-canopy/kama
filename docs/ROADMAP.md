...
- **Bare-metal MCU (Arduino AVR, Cortex-M: Pi Pico/RP2040 · Arduino Zero/Nano 33, ESP32):** *freestanding*
  ...
  | **Globals / statics** | MCU code lives on module-level state (peripheral handles, ISR flags, flash tables); new language surface with deterministic zero/const init. Also **`const` data in flash** (`.rodata`; on **AVR** the Harvard `PROGMEM` wart). **Build it *per-isolate by construction*** — the concurrency spec ([design/concurrency.md](design/concurrency.md) §"three sharing seams") **pins this semantic**: a module `static` is per-isolate (plain C `static` = zero cost on a single-core MCU; `_Thread_local` on multicore native; automatic on wasm), so the same declaration is race-free under the threading model with no extra work — cross-isolate sharing stays on the `Atomic<T>` seam. The concurrency campaign pinned the rule; **this MCU task is where statics are actually built to it** |
