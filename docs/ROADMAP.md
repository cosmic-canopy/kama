...
| KR-55 | **`type extern enum` / `type expose enum` — an enum at the C boundary** — `type extern enum` is accepted and INERT today (it numbers variants itself and hands C the wrong value), and an exposed enum's constants are an ABI promise nothing marks. RULED: the value marker pair reaches enums — extern variants are C's constant names verbatim with no values and are checked against the header; expose enums spell every value and publish module-qualified; a plain enum at the boundary is refused | M | [§2](ROADMAP_DETAIL.md#s2) |
