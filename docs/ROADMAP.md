...
| KR-89 | **Use-after-move is not diagnosed for a `give` into a DECLARATION INITIALIZER** — `string b = give a;` then reading `a` builds clean and yields `""`, silently, though every other position and every other owning type is refused; kama states the guarantee on its front page. ⚠️ **`BindableFunctionPtr` has the same hole and calling the moved-from value SEGFAULTS** (measured), so there it is the only thing between the user and a crash. Probed: the read side is complete and the emitter already nulls the source — one `markMoved` is missing | S | [§2](ROADMAP_DETAIL.md#s2) |
