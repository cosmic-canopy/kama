...
| KR-88 | **A module-scoped declaration reaches C outside kama's registers** — SPEC § *C names* says every name kama owns is `k_`/`kama_`/`KAMA_`, but a declaration in a module emits as `<project>__<module>__name` (`nsbasic__graphics__scale`, `std__math__sqrt__float64`, now `core__println`), so a project whose name spells a C macro prefix is exposed and the "disjoint by proof" claim is false. Found 2026-09-22; measure the macro exposure, then prefix or restate | S? | [§2](ROADMAP_DETAIL.md#s2) |
