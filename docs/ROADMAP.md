...
| KR-76 | **On Windows every short `std::time::sleep` costs one 15.6 ms scheduler tick** — `kama_sleep_ns` (`include/kama_time.h`) is `Sleep(ms rounded up)`, and nothing in kama raises the timer resolution, so a frame loop's 8 ms nap and a poll loop's 10 ms wait both take ~16 ms and any program pacing itself with `sleep` runs at half speed. ⚠️ **Not a contract break** — SPEC promises "at least `d`" and the round-up is deliberate (a 1 ns sleep must not be `Sleep(0)`) — so this is reach, not a bug. Ask: a `CreateWaitableTimerExW(…, CREATE_WAITABLE_TIMER_HIGH_RESOLUTION, …)` arm falling back to `Sleep` where the flag is refused (Win10 1803+); **not** `timeBeginPeriod(1)`, which is process-global and a library may not spend a caller's power. Filed by the first consumer as KG-36 and re-measured here. **Windows box** | S | [§1](ROADMAP_DETAIL.md#s1) |
