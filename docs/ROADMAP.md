...
| KR-59 | **A payload-less enum is always a C integer** — today a method, a contract or `@generate` PROMOTES it to a tagged struct, silently changing its `sizeof`, its C type and its ABI; that promotion already broke six scalar operations once (`0.9.234`) and is why a `type extern enum`/`type expose enum` may not have members. Lower every payload-less enum to its integer, with methods as free functions scoped to its name (`Color__isWarm(Color self)`) through the by-value receiver `type intrinsic` already uses; delete the promotion path. Payload enums stay structs. No kama source change; a C ABI change for enums with members, so before 1.0 | L | [§2](ROADMAP_DETAIL.md#s2) |
