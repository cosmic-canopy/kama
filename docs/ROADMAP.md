...
| 9 | **A C-compiler error drifts past the construct it is in** — clang counts forward from the last `#line`, so a long lowering leaves the construct and, on a short file, the file: measured at `vm.kama:18` in a **10-line** file | S | `kama check` does not type-check expressions, so clang IS the diagnostic for a large class of real errors | [§2](ROADMAP_DETAIL.md#s2) |
