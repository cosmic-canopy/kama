...
| # | item | size | detail |
|---|---|---|---|
| 1 | **A read-only `View` — `ConstView<T>` and the `view()`/`viewMut()` split** — the pointer seam has its read-only half (`UnsafeConstPtr<T>`, `dataPtr()`/`dataPtrMut()`) and the language its read-only place (`const ref T`, 0.9.210); the view does not: `view()` is non-const, so a `const ref DynamicArray<uint8>` cannot produce a window at all, and every stdlib FFI wrapper takes `View<uint8>` by value for that reason alone. `ConstView` indexes through `const ref T operator[]` (any element), narrows from `View<T>` implicitly. Rust `&[T]`/`&mut [T]`, C# `ReadOnlySpan`/`Span` | L | [§2](ROADMAP_DETAIL.md#s2) |
| 2 | **Audit every "waiting on a consumer" deferral** — kama is a general-purpose systems language: a feature a consumer will need is decided, not deferred until the corpus asks for it. One verdict per item (schedule · genuinely optional · non-goal, with the reason); each scheduled one becomes its own row. Seeded by the const-pointer arc and the read-only place, both exactly such deferrals (the rule now lives in AGENTS.md § *Working style*) | M | [§3](ROADMAP_DETAIL.md#s3) |
