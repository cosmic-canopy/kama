...
`volatile` and `export` are the **only** two keywords not yet implemented. Both are genuine keywords today
...
- **`export`** has a real future role at the cstar→host boundary (WASM module exports for the browser engine;
  the scripting host interface), distinct from in-language `public`/`private`.
