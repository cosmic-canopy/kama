...
// inputs so the checksum matches every backend). Plain structs + field-by-field ops, same as kama's lib.
#include <stdint.h>

typedef struct { float x, y, z, w; } V4;
typedef struct { V4 c0, c1, c2, c3; } M4;
...
static M4 m4_mul(M4 a, M4 b){
  return (M4){ m4_transform(a, b.c0), m4_transform(a, b.c1),
