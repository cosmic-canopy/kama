...
#
# The Makefile builds into build/<os>-<arch>/ so a host (mach-o) and a container (ELF) build coexist,
...
# root symlink (an installed/packaged tree has no build/ at all). An externally set $KAMA always wins.
if [ -z "${KAMA:-}" ]; then
    KAMA="$ROOT/build/$(uname -s)-$(uname -m)/kama"
