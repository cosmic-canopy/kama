...
cat >"$tmp/ccount" <<'EOF'
#!/bin/sh
# `-v` is not a compile: the driver asks each distinct `--cc` string which family it belongs to, once per
# build, because the warning flags it emits are spelled differently by clang and gcc and are a hard error
# on the wrong one (KR-72). Answering it the way a real compiler does keeps this guard counting COMPILES —
# the thing it is about — instead of drifting by one every time the driver asks a question.
case " $* " in
    *" -v "*) echo "cc (kama check-build-jobs shim) 0.0.0"; exit 0 ;;
esac
